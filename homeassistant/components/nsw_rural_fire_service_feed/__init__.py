...
async def async_setup(hass, config):
    """Set up the NSW Rural Fire Service Feeds component."""
...
async def async_setup_entry(hass, config_entry):
    """Set up the NSW Rural Fire Service Feeds component as config entry."""
    hass.data.setdefault(DOMAIN, {})
    feeds = hass.data[DOMAIN].setdefault(FEED, {})

    radius = config_entry.data[CONF_RADIUS]
    # Create feed entity manager for all platforms.
    manager = NswRuralFireServiceFeedEntityManager(hass, config_entry, radius)
...
class NswRuralFireServiceFeedEntityManager:
    """Feed Entity Manager for NSW Rural Fire Service feed."""
    ...
    async def _update_entity(self, external_id):
        """Update entity."""
        async_dispatcher_send(
            self._hass, f"nsw_rural_fire_service_feed_update_{external_id}"
        )

    async def _remove_entity(self, external_id):
        """Remove entity."""
        async_dispatcher_send(
            self._hass, f"nsw_rural_fire_service_feed_delete_{external_id}"
        )
    ...
    async def _status_update(self, status_info):
        ...
        async_dispatcher_send(
            self._hass, f"nsw_rural_fire_service_feed_status_{self._config_entry_id}"
        )
